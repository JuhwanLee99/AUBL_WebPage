...
4. **⭐ 올스타·루키 팬 투표**
   ...
   * 후보·투표 화면은 실제 `visualViewport` 높이에 고정한 상단바 없는 모바일 전용 레이아웃이며, 페이지 자체의 세로 스크롤 없이 원통 카드와 확대 카드를 탐색
   * 약한 캐러셀 스와이프는 1장, 충분히 강한 스와이프는 최대 2장씩 이동하고, 확대 화면은 카드 덱에서 한 장씩 꺼내고 다시 쌓는 전환을 사용
