...
## Completed-game details (adapter `2026.09.05.9`)
...
- Team tabs are the unique direct-child control whose two labels exactly match the line score, with exactly one selected background. Optional scorer information cannot shift a hardcoded tab index; its text is not retained. Explicit empty jersey parentheses `()` mean `null`, while malformed labels still fail.
