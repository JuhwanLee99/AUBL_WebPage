...
const WEB_MEMBER_SECTIONS: ManualSection[] = [
  ...
  },
  {
    ...
    accent: '#22C55E',
    items: [
      { heading: '커뮤니티 메인 (/community)', bullets: ['갤러리와 공지 대시보드를 한눈에 확인합니다.'] },
...
export default function UserManualPage() {
  ...
  return (
    <div style={{ display: 'grid', gap: '28px' }} ref={pageRef}>
      ...
      >
        <p style={{ margin: 0 }}>
          문서 버전: 2026-02-17 · {platform === '웹' ? '일부 관리 기능은 데스크톱 또는 태블릿 가로 모드를 권장합니다.' : '일부 관리 기능은 앱 내 WebView로 웹 관리 화면에 연결됩니다.'}
        </p>
      </section>
    </div>
  );
