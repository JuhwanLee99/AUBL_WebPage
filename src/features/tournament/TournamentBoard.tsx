...
export default function TournamentBoard({ config }: { config: TournamentConfig }) {
  ...
  return <div className="t26">
    ...
    <p className="t26-help">조·순위 코드는 진출 자리입니다. 팀명은 운영진이 별도로 확정하며, 경기 결과 표시는 공식 누적 기록을 변경하지 않습니다. 좁은 화면에서는 대진표를 좌우로 이동하세요.</p>
        ...
        {division.matches.map(match => {
          ...
          return <article className={`t26-match ${match.round === 'final' ? 'is-final' : ''}`} key={match.id}
            ...
            {pair.map((seed, side) => <div key={side} className={`t26-team ${seed && seed === match.winner ? 'is-winner' : ''}`}>
              <span className="t26-seed">{seed || '-'}</span><strong title={entrantLabel(division, seed)}>{entrantLabel(division, seed)}</strong><b>{match.scores[side] ?? ''}</b>
            </div>)}
            <footer>{scheduleLabel(match.startTime)}</footer>
          </article>;
        })}
        <div className="t26-champion"><span>우승</span><strong>{final?.winner ? entrantLabel(division, final.winner) : '결승 승자'}</strong></div>
      </div>
    </div>
    <details className="t26-schedules" open><summary>{divisionLabel[selected]} 경기 일정 · 결과</summary>
      <div className="t26-table-scroll"><table><caption className="t26-help">시간은 한국 시간(KST) 기준입니다.</caption>
        ...
        <tbody>{schedule.map(match => { const pair = entrants(division, match); return <tr key={match.id}>
          <td>{roundLabel[match.round]} {match.index + 1}</td><td>{pair.map(seed => seed ? `${seed} ${entrantLabel(division, seed)}` : '이전 경기 승자').join(' / ')}</td>
