...
export default function AdminIntroPage() {
  ...
  const serializeHistory = (items = intro.historyHighlights) => items.map((h) => `${h.title} | ${h.desc} | ${h.accent}`).join('\n');
  const serializeGovernance = (items = intro.governance) => items.map((g) => `${g.label} | ${g.value} | ${g.detail}`).join('\n');
  const serializeStructure = (items = intro.structureCards) => items.map((s) => `${s.title} | ${s.points.join('; ')}`).join('\n');
  const serializePostseason = (items = intro.postseasonMatches) => items.map((p) => `${p.title} | ${p.matchups.join('; ')}`).join('\n');
  const serializeMetrics = (items = intro.heroMetrics) => items.map((m) => `${m.label} | ${m.value} | ${m.note}`).join('\n');
  ...
  const preview = useMemo(
    ...
    [tagline, heroSubtitle, heroTitle, heroDescription, historyDraft, governanceDraft, structureDraft, postseasonDraft, metricsDraft],
  );
