                  ...
                  {socialConsentTab === 'terms' ? (
                    <>
                      {[
                        ...
                        { title: '제4조 (회원 가입 및 탈퇴)', items: ['회원 가입은 이메일·비밀번호 등록 또는 Google/Apple 계정을 통한 소셜 로그인으로 이루어지며, 가입 시 본 약관 및 개인정보 처리방침에 동의한 것으로 간주합니다.', '회원은 언제든지 앱 내 "더보기 → 계정 → 회원 탈퇴"에서 탈퇴를 요청할 수 있으며, 웹 계정 삭제 안내 페이지(https://aubl.club/account-deletion)에서도 삭제 절차를 확인할 수 있습니다. 탈퇴 시 회원 정보는 지체 없이 파기하되, 투표 이의 처리 및 법령상 보존이 필요한 기록은 개인정보 처리방침의 기간을 따릅니다.'] },
                        { title: '제5조 (서비스의 제공 및 변경)', items: ['AUBL은 경기 일정·결과 조회, 실시간 문자중계, 선수 기록 열람, 커뮤니티, 팀 관리, 푸시 알림, 올스타·루키 후보 공개 및 투표 서비스를 제공합니다.', '올스타·루키 투표는 이벤트별로 공지된 참여 주기(예: 계정당 1회 또는 계정당 1일 1회), 후보와 포지션별 선택 수, 투표 기간에 따라 운영됩니다.', '종료 경기 및 과거 시즌 기록 데이터는 AUBL 백엔드 API(api.aubl.club)와 운영 MariaDB를 통해 제공될 수 있으며, API 전송 구간에 Cloudflare 인프라가 사용될 수 있습니다.', '서비스는 무료로 제공되며, 향후 유료 서비스 도입 시 별도 고지 후 동의를 받습니다.'] },
                        { title: '제7조 (회원의 의무)', items: ['타인의 개인정보 도용, 허위 정보 등록, 서비스 운영 방해, 욕설·비방·음란물 게시, 상업적 광고 게시, 무단 크롤링·스크래핑을 금지합니다.', '여러 계정, 자동화 도구 또는 인증 우회 수단으로 투표 제한을 회피하거나 결과를 조작해서는 안 됩니다.', '위반 시 AUBL은 사전 통지 없이 서비스 이용을 제한하거나 회원 자격을 박탈할 수 있습니다.'] },
                    ...
                    <>
                      {[
                        { title: '1. 개인정보의 수집 항목 및 수집 방법', items: ['수집 항목: 이메일 주소, 이름(소셜 로그인 시 제공되는 경우), 계정 고유 식별자(UID)', '자동 수집: 기기 식별 정보, 앱 버전, OS 종류 및 버전, FCM 푸시 토큰', '커뮤니티(건의/문의) 이용 시 수집 항목: 게시글/댓글 내용, 작성 시각, 작성자 식별 정보(UID, 표시명)', '올스타·루키 투표 참여 시 처리 항목: Firebase 인증에서 확인한 Google 계정 식별값을 HMAC 처리한 가명 중복 방지 키, 이벤트·부문·후보 버전, 선택 후보, 제출 시각 및 적용 투표 주기', '수집 방법: 이메일·비밀번호 회원가입 또는 Google/Apple 소셜 로그인'] },
                        { title: '2. 개인정보의 수집 및 이용 목적', items: ['회원 식별 및 가입 의사 확인', '리그 경기 일정·결과·기록 조회 서비스 제공', '커뮤니티 게시글 작성·관리', '건의/문의 접수, 답변, 처리 상태 안내', '팀 공지사항 및 경기 알림(푸시 알림) 발송', '올스타·루키 투표 참여 자격 확인, 계정별 중복 투표 방지, 결과 집계 및 이의 처리'] },
                        { title: '3. 개인정보의 보유 및 이용 기간', items: ['회원 정보는 회원 탈퇴 시까지 보유하며, 탈퇴 요청 시 지체 없이 파기합니다. 다만 투표 기록과 법령상 보존 대상은 각 보유 기준을 따릅니다.', '투표 중복 방지 키와 선택 내역은 결과 확정 및 이의 처리에 필요한 기간 동안 보관한 뒤 삭제하거나 개인을 식별할 수 없는 집계 정보만 남깁니다.', '통신비밀보호법에 의한 로그 기록: 3개월'] },
