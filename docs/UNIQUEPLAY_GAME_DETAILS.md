...
### 상세 기록 / 문자중계 스타일 비교 (2026-09-05)

- 공식 상세를 조회한 뒤 상단 토글로 두 화면을 전환한다. 공식 데이터는 항상 `상세 기록`을 기본으로 시작한다. 선택은 현재 경기 화면을 보는 동안만 유지하며 새로고침·다른 경기·재진입 시 초기화된다. 과거 `aubl:official-game-view:v1` 저장값은 읽거나 쓰지 않아 이전 문자중계 선택이 기본값을 덮어쓰지 않는다.
- `상세 기록`은 현재 경기 헤더와 전체 너비 공식 기록표를 유지한다. 최초 구현의 별도 재구성안은 사용자 피드백으로 제거했다. `문자중계 스타일`은 운영 `/scoreboard-text`의 기존 네이비 전광판, 중앙 투수/타자 정보, 현재 타석 카드, 경기 요약, 문자중계 다시보기, 양 팀 기록표, 교체 out 영역, 기록지 탭의 실제 컴포넌트와 CSS를 재사용한다.
- 기존 `ScoreboardPanel`의 렌더링만 `ScoreboardPanelDisplay`로 분리하고 연결 계층은 그대로 둔다. `NowPlayingCard`, `PostGameSummary`, `LiveFeed`, `CsvRecordPreview`도 기존 구현을 공유한다. 새 `officialLegacyRecord`는 공개 응답을 표시용 행으로 변환할 뿐 Firestore 기록원 상태에 주입하지 않는다.
- 두 버전은 같은 공개 API 응답을 사용한다. 기존 형식의 선수 표는 원천에 없는 타석수·장타 종류·볼넷/사구 분리·투구수·출루율을 `—`로 표시하며 문자열에서 역산하지 않는다. 원본의 타율·ERA·1.2이닝 표기를 보존한다. 화면 전환 자체는 API 재조회, 수집, 게시, Firestore 쓰기를 하지 않는다.
- 문자중계 목록은 기존 이닝 펼치기/접기 UI를 유지하면서 `officialBattingFlow`에서 타석 결과를 분리하고 이전 이닝의 다음 타순을 이어 간다. 같은 이닝의 반복 타석은 9번 다음 1번으로 순환한다. 볼넷·사구·타구 결과와 주루·교체 표기를 구분하며 후자는 별도 타석으로 세지 않는다. 이는 원본 결과와 타순으로 재구성한 순서이며 투구별 실제 이벤트 로그가 아니다. 초말·주자 위치·현재 타자·볼카운트·통계는 역산하지 않는다.
- 미분류 표기, 타순 누락·범위 오류, 이닝 누락, 필요한 다음 타자의 기록 부재, 같은 이닝 같은 타순에 여러 선수의 타석이 있는 교체 모호성은 `타순 확인 필요`로 표시한다. 해당 이닝 이후 연결을 강제 추정하지 않고 원본 표기로 전환한다. 이닝 정보가 없는 결과는 `이닝 미상`이다. 빈 교체 선수 행은 타석을 만들지 않으며 타석 없는 주루·교체 행은 시점 미상 원문으로 보존한다. 실시간 정보·교체 여부는 `미제공`으로 구분하고 기본값 0을 노출하지 않는다.
- 공식 보기의 전광판 확대는 같은 공식 표시 데이터를 대화상자에서 확대한 것이다. 문자중계 불러오기는 게시된 이닝 표시를 여는 동작이며 수집을 시작하지 않는다. 기록지 다운로드는 공개 값만 CSV로 내보내며 수식 주입을 방어한다.
- 기존 AUBL 실시간 문자중계·기록원·연습경기 경로는 유지한다. 공식 상세가 없는 경기를 토글로 수기 기록에 강제 연결하지 않는다.
- 테스트: `npm run test:official-views` (렌더링·변환·타순 연결·기존 실시간 회귀 15개), `node scripts/test-official-game-views.mjs --browser` (총 38개; 별도 로컬 fixture 서버와 Playwright). fixture 브라우저 검증은 운영 API나 Firebase를 호출하지 않는다. 360·390·768·1280·1920px의 라이트/다크·토글·다시보기·기록지 탭·확대와 과거 저장값 무시·경기 전환 초기화를 검사한다.
- 타순 회귀 fixture는 `up-f1e054d7678bf5467580de7d`의 공개 표기를 사용한다. 고려대 `1회 1~3번 → 2회 4~9번·1~8번(14타석) → 3회 9·1~3번 → 4회 4~7번 → 5회 8·9·1~3번`, 경희대의 별도 순환, 도루자·선행 폭투, 반복 타석, 빈 교체 행, 이닝 사이 교체, 모호한 교체, 미분류 표기, 3회전 타자일순, 원본/CSV/통계 불변성을 검증한다.

실시간 경기는 동일한 Firestore 경기의 진행 상태를 유지한다. 공식 경기의 실시간 경로는 같은 경기의 기록 시작/실제 이벤트가 있고 사후 수동 입력 모드가 아니며, Firestore 진행 중·공식 상세 미수집·DB 예정/진행 중일 때만 사용한다. 일정에 진행 중이라고 표시된 것만으로 빈 문자중계 화면을 기본값으로 고르지 않는다. 종료·취소·검수 필요 상태나 다른 경기의 활성 상태로 자동 대체하지 않으며, 게시된 공식 기록과 보관된 수기 기록의 우선순위는 유지한다.
